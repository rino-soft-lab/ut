# Usefull Things (Полезные мелочи)
...
> Если необходима установка компонентов, pre-Setup - сохранит все необходимые (для продолжения настройки) данные, и для продолжения процесса - достаточно будет (после перезагрузки интернет-центра) подключиться к entware и запустить:

```
prs
```

> без параметров (скрипт завершит процесс настройки и удалит все следы своего прибывания)...
